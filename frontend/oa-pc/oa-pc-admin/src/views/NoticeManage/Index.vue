<template>
  <div class="page-container">
    ...
    <!-- ===== 阅读统计对话框 ===== -->
    <el-dialog v-model="showStatsDialog" title="公告阅读统计" width="750px">
      <!-- 总览统计 -->
      ...
      <el-table :data="statsData" style="width:100%;" max-height="350" stripe>
        ...
        <el-table-column prop="title" label="公告标题" min-width="200">
          <template #default="{ row }">
            <el-link type="primary" @click="openDetailFromStats(row.id)">
        ...
        <el-table-column label="阅读率" width="220">
          <template #default="{ row }">
...
const allData = ref([])
const statsData = ref([])
const statsTotalEmployees = ref(0)
...
const avgReadRate = computed(() => {
  ...
  return Math.round((total / (published.length * Math.max(statsTotalEmployees.value, 1))) * 100)
})
...
// ==================== 合并阅读统计数据 ====================
const mergeReadStats = (list, stats) => {
  // 创建统计映射
  const statsMap = {}
  stats.forEach(stat => {
    statsMap[stat.id] = {
  ...
  return list.map(item => ({
    ...item,
    readCount: statsMap[item.id]?.readCount || 0,
    totalCount: statsMap[item.id]?.totalCount || 0,
    readRate: statsMap[item.id]?.readRate || 0
  }))
}
...
const loadData = async () => {
  ...
  try {
    ...
    try {
      const statsRes = await getAnnouncementReadStats()
      if (statsRes.code === 0 && statsRes.data) {
        statsList = statsRes.data.stats || []
        totalEmployees = statsRes.data.totalEmployees || 0
        statsData.value = statsList
        statsTotalEmployees.value = totalEmployees
      }
    } catch (statsError) {
      console.warn('加载阅读统计失败:', statsError)
    }
    ...
    if (response && response.code === 0) {
      ...
      // 3. 合并阅读统计数据
      list = mergeReadStats(list, statsList)
...
// 从统计弹窗打开详情
const openDetailFromStats = async (id) => {
  try {
