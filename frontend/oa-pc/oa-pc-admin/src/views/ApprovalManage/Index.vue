...
const reportStats = ref([
  ...
  { title: '待审批', value: 0, color: '#e6a23c' },
  { title: '已通过', value: 0, color: '#67c23a' },
  { title: '通过率', value: '0%', color: '#67c23a' }
])
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      const result = Object.values(deptMap).map(item => {
        const total = item.totalCount - item.pendingCount || 1
        const approvalRate = Math.round((item.approvedCount / total) * 100)
...
const updateReportStats = (data) => {
  ...
  reportStats.value[2].value = approved
  reportStats.value[3].value = total > 0 
    ? Math.round((approved / (total - pending || 1)) * 100) + '%' 
    : '0%'
}
