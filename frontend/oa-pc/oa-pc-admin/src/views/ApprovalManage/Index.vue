...
const loadPendingList = async () => {
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      ElMessage.warning(res.msg || '加载待审批列表失败')
    }
  } catch (error) {
...
const loadAllList = async () => {
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      allList.value = list
      allTotal.value = data.total || list.length
      stats.value[3].value = allTotal.value
    } else {
      ElMessage.warning(res.msg || '加载全部列表失败')
    }
  } catch (error) {
    console.error('加载全部列表失败:', error)
    ElMessage.error('加载全部列表失败')
  } finally {
...
const exportAllReport = async () => {
  ...
  try {
    ...
    let allData = res.data?.records || res.data?.list || []
