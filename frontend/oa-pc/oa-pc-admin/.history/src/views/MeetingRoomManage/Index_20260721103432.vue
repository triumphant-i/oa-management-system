    ...
    <el-card>
      <el-table :data="tableData" style="width:100%;" v-loading="loading" stripe>
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="{ row }">
            <!-- ✅ 预约按钮 - 所有登录用户都可以预约 -->
            <el-button size="small" type="success" plain @click="openBookingDialog(row)">
    ...
    >
      <el-form :model="bookingForm" :rules="bookingRules" ref="bookingFormRef" label-width="100px">
        <el-form-item label="会议室">
          <span>{{ bookingForm.roomName }}</span>
        </el-form-item>
        ...
        <el-form-item label="参与人">
          <el-select 
            v-model="bookingForm.participantIds" 
            multiple 
            filterable 
            placeholder="请选择参与人"
            style="width:100%;"
          >
            <el-option 
              ...
              :key="emp.id" 
              :label="emp.name + ' (' + (emp.departmentName || '未分配') + ')'" 
              :value="emp.id"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="bookingForm.remark" type="textarea" rows="2" placeholder="请输入备注信息" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="bookingVisible = false">取消</el-button>
        <el-button type="primary" @click="handleBookingSubmit" :loading="bookingLoading">
          确认预约
        </el-button>
      </template>
    </el-dialog>
...
import { Plus, Refresh, Edit, Search, Switch, Delete, Calendar } from '@element-plus/icons-vue'
...
const updateStats = () => {
  const available = tableData.value.filter(r => r.status === '可用').length
  const occupied = tableData.value.filter(r => r.status === '使用中').length
  const maintenance = tableData.value.filter(r => r.status === '维护中').length
...
const loadEmployees = async () => {
  try {
    const res = await getEmployeePage(1, 999)
    if (res.code === 0) {
...
const openBookingDialog = (row) => {
  bookingForm.roomId = row.id
  bookingForm.roomName = row.name
  bookingForm.title = ''
  bookingForm.startTime = ''
  bookingForm.endTime = ''
  bookingForm.participantIds = []
  bookingForm.remark = ''
...
onMounted(() => {
  ...
  if (userInfo.id) {
    ...
    currentUser.value.name = userInfo.name || '管理员'
  }
