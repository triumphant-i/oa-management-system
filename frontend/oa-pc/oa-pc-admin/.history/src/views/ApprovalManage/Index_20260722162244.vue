  ...
  try {
    ...
    if (res.code === 0) {
      ...
      const result = Object.values(deptMap).map(item => {
        // 审批率 = (已通过 + 已拒绝) / (总申请 - 待审批) * 100%
        // 即：已审批的申请占已处理申请的比例
        const processed = item.totalCount - item.pendingCount
        const approvedAndRejected = item.approvedCount + item.rejectedCount
        const approvalRate = processed > 0 
          ? Math.round((approvedAndRejected / processed) * 100) 
          : 0
...
const updateReportStats = (data) => {
  ...
  reportStats.value[3].value = processed > 0 
    ? Math.round((approvedAndRejected / processed) * 100) + '%' 
    : '0%'
}
