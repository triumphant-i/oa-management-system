...
// ==================== API 导入 ====================
import { getMyMessages, markMessageRead, markAllMessagesRead, deleteReadMessages } from '@/api/message'
...
const loadMessages = async () => {
  ...
  try {
    ...
    const res = await getMyMessages({
      receiverId: parseInt(employeeId),
      bizTypes: ['approval', 'meeting'],
    ...
    if (res.code === 0) {
      const data = res.data || {}
      messages.value = data.records || data.data || data.list || []
      total.value = data.total || messages.value.length
      updateStats()
    } else {
      ElMessage.error(res.msg || '加载消息失败')
    }
  } catch (error) {
...
const markAllRead = async () => {
  try {
    ...
    const res = await markAllMessagesRead(parseInt(employeeId))
    if (res.code === 0) {
...
const deleteAllRead = async () => {
  try {
    ...
    const res = await deleteReadMessages(parseInt(employeeId))
    if (res.code === 0) {
