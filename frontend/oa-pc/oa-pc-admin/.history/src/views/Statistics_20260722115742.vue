...
const loadEmployeeOptions = async () => {
  try {
    ...
    if (res.code === 0) {
      employeeOptions.value = res.data?.data || res.data || []
    }
  } catch (error) {
...
const updateOverviewFromAttendance = (data) => {
  try {
    ...
    data.forEach(item => {
      ...
      if (item.overtimeHours !== undefined && item.overtimeHours !== null) {
        totalOvertime += item.overtimeHours
      }
    })
    
    const avgRate = count > 0 ? (totalRate / count) : 0
    const avgOvertime = data.length > 0 ? (totalOvertime / data.length) : 0
    
    overview.value = overview.value.map(item => {
      if (item.title.includes('出勤率')) {
        return { ...item, value: avgRate.toFixed(1) + '%', sub: '平均出勤率' }
      }
      if (item.title.includes('加班')) {
        return { ...item, value: avgOvertime.toFixed(1) + 'h', sub: '平均加班时长' }
      }
      return item
    })
  } catch (error) {
