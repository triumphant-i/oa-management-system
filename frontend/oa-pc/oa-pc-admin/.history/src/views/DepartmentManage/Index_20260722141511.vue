      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="部门主管">
          ...
          <div style="font-size:12px;color:#999;margin-top:4px;">
            ⭐ 标记表示该员工已是部门主管
            <span v-if="!form.departmentId" style="color:#e6a23c;">请先选择部门</span>
...
const departmentEmployees = computed(() => {
  ...
  return allEmployees.value.filter(emp => 
    emp.departmentId === form.departmentId && emp.status === '在职'
  )
})
...
const form = reactive({
  ...
  name: '',
  parentId: 0,
  departmentId: null,  // 当前部门ID（用于筛选员工）
  managerId: null,
  description: ''
})
...
const openEditDialog = (row) => {
  ...
  form.departmentId = row.id  // 当前部门ID
