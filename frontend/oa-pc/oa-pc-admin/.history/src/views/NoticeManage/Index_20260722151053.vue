  ...
  try {
    ...
    if (response && response.code === 0) {
      ...
      if (data && data.data && Array.isArray(data.data)) {
        ...
        console.log('从 PageVO 解析: total=', total.value, ', data.length=', tableData.value.length)
      } else if (Array.isArray(data)) {
        // 直接数组（分类查询返回）
        tableData.value = data
        total.value = data.length
        console.log('从数组解析: 共', total.value, '条')
      } else if (data && data.records && Array.isArray(data.records)) {
        // MyBatis Plus 分页结构
        tableData.value = data.records
        total.value = data.total || data.records.length
        console.log('从 MyBatis Plus 分页解析: total=', total.value)
      } else {
        // 尝试其他结构
