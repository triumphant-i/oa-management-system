      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            ...
            />
            <el-select 
              v-model="attendanceParams.employeeId" 
              placeholder="选择员工" 
              clearable 
              filterable 
              @change="loadAttendanceStats" 
...
const loadEmployeeOptions = async () => {
  try {
    ...
    if (res.code === 0) {
      ...
      employeeOptions.value = Array.isArray(data) ? data : []
    }
  } catch (error) {
...
const loadAttendanceStats = async () => {
  ...
  try {
    ...
    // 后端要求 employeeId 是 Integer 类型，不能为 null
    // 如果 employeeId 为 null，传 0 表示查询全部员工
    const employeeId = attendanceParams.employeeId || 0

    const res = await getAttendanceStatistics(employeeId, year, month)
